...
public interface PedidoClient {
    ...
    @CircuitBreaker(name = "pedido-service", fallbackMethod = "fallbackBuscarEmail")
    @GetMapping("/{id}/email")
    String buscarEmailPorPedidoId(@PathVariable("id") Long pedidoId);
    ...
    default String fallbackBuscarEmail(Long pedidoId, Throwable throwable) {
        PedidoClient.logger.warn("⚠️ Erro ao buscar email do pedido {} - Fallback ativado: {}",
                pedidoId, throwable.getMessage());
        return "rickrickelmy98@gmail.com"; // Fallback para o email padrão
    }
